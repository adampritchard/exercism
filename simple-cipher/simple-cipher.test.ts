...
describe('Random key generation', () => {
  ...
  it('random key uses chars from a to z', () => {
    let foundA = false
    let foundZ = false
    for (let i = 0; i <= 1000; i++) {
      const key = new SimpleCipher().key
      if (key.includes('a')) foundA = true
      if (key.includes('z')) foundZ = true

      if (foundA && foundZ) break
    }

    expect(foundA).toBeTruthy()
    expect(foundZ).toBeTruthy()
  })
})
